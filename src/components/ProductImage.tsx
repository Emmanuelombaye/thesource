...
export default function ProductImage({ product, size = "card", priority = false }: ProductImageProps) {
  ...
  if (!src || failed) {
    return (
      <div className={`${styles.fallback} ${isDetail ? styles.fallbackDetail : ""}`}>
  ...
  return (
    <div
      className={`${styles.frame} ${isDetail ? styles.frameDetail : styles.frameCard} ${isCollection && !isDetail ? styles.frameCollection : ""}`}
    >
      <Image
        src={src}
        alt={`${product.name}${product.amount && product.amount !== "—" ? ` ${product.amount}` : ""}`}
        fill
        className={className}
        sizes={sizes}
        priority={priority}
